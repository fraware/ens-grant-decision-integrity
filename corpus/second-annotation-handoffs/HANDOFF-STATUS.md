...
Operator / annotator procedure: `ANNOTATOR-CHECKLIST.md` (this directory) and `corpus/DOUBLE-ANNOTATION.md`.

## Frozen handoffs (exact UTF-8 LF bytes)

Verified 2026-08-25 against current `case.json` via `scripts/second_annotation.py prepare` (regenerated bytes must match these hashes). Working-tree CRLF corruption invalidates hashes; `corpus/**/*.json` is forced to LF via `.gitattributes`.
...
Hashes are over the exact handoff file bytes as prepared by `scripts/second_annotation.py` (LF newlines). Do not reformat JSON before hashing or verifying.
...
3. On return, verify without modifying the case (`second_annotation.py verify` — commands in the checklist).
