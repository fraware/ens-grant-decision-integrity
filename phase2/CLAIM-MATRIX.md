...
| C4 Run attribution | Signer asserts this run used the bound commitment, input snapshots, environment, and output digest. | that the signer actually used that configuration or that the output is sound; that the signing key is authorized by an external operator policy (see C4A) |
| C4A Authorized signer | The verified run signature key/identity is authorized by the external verifier trust policy for the declared operator role within the policy validity window. | operator honesty; institutional funding authority; C4 when no signature is present |
