...
def _require_unique_strings(value: Any, *, label: str, code: str) -> list[str]:
...
def _study_parameters(plan: dict[str, Any]) -> tuple[int, int, list[str], list[str], float]:
    ...
    if not isinstance(maximum, int) or isinstance(maximum, bool) or maximum < minimum:
        raise StudyStatusError(
            "sampling.targetMaximumCases must be an integer greater than or equal to targetMinimumCases",
            code="STUDY001",
        )
    ...
    conditional = _require_unique_strings(
    ...
    if overlap:
        raise StudyStatusError(
            f"required and conditional strata must be disjoint: {overlap}",
            code="STUDY001",
        )
    ...
    if not isinstance(fraction, (int, float)) or isinstance(fraction, bool) or not 0 <= fraction <= 1:
        raise StudyStatusError(
            "annotation.doubleAnnotationMinimumFraction must be between 0 and 1",
            code="STUDY001",
        )
...
def _case_summary(case: dict[str, Any], *, source: str) -> dict[str, Any]:
    if case.get("template") is True:
        raise StudyStatusError(
            f"empirical case directory contains template=true case: {source}",
            code="STUDY004",
        )
    ...
    strata = _require_unique_strings(
        selection.get("strata"),
        label=f"{case_id} selection.strata",
        code="STUDY003",
    )
...
def discover_cases(cases_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in paths:
        ...
        summaries.append(_case_summary(case, source=str(path.relative_to(ROOT))))
    return summaries


def compute_study_status(plan: dict[str, Any], cases: list[dict[str, Any]]) -> dict[str, Any]:
    minimum, maximum, required, conditional, double_fraction_min = _study_parameters(plan)
    ...
    coverage_sorted = {stratum: sorted(ids) for stratum, ids in sorted(coverage.items())}

    case_count = len(normalized)
    double_count = sum(1 for case in normalized if case["doubleAnnotation"])
    double_fraction = 0.0 if case_count == 0 else round(double_count / case_count, 6)
    needed_current = 0 if case_count == 0 else math.ceil(double_fraction_min * case_count)
    needed_at_minimum = math.ceil(double_fraction_min * minimum)

    required_missing = [stratum for stratum in required if stratum not in coverage]
    ...
    declared = set(required) | set(conditional)
    extra = sorted(set(coverage) - declared)

    gates = {
        "minimumCaseCountMet": case_count >= minimum,
        "maximumCaseCountNotExceeded": case_count <= maximum,
        "requiredStrataDeclaredCoverageMet": not required_missing,
        "doubleAnnotationFractionMet": double_fraction >= double_fraction_min,
    }
    ...
    if case_count < minimum:
        blockers.append(f"case-count: {case_count}/{minimum} minimum")
    if case_count > maximum:
        blockers.append(f"case-count: {case_count} exceeds predeclared maximum {maximum}")
    if required_missing:
        blockers.append(
    ...
    if double_fraction < double_fraction_min:
        blockers.append(
            f"double-annotation: {double_count}/{case_count} cases ({double_fraction:.6f}) "
            f"below minimum fraction {double_fraction_min:.6f}"
        )
    ...
    return {
        ...
        "cases": [
            {
                "caseId": case["caseId"],
                "strata": case["strata"],
                "doubleAnnotation": case["doubleAnnotation"],
            }
        ...
        "strata": {
            ...
            "requiredUnresolved": required_missing,
            "conditionalCovered": conditional_covered,
            "conditionalUncovered": conditional_uncovered,
            "additionalObserved": extra,
        },
        "doubleAnnotation": {
            "caseCount": double_count,
            "observedFraction": double_fraction,
            "minimumFraction": double_fraction_min,
            "minimumCasesNeededForCurrentCorpus": needed_current,
            "minimumCasesNeededAtTargetMinimum": needed_at_minimum,
        },
        "gates": gates,
        "blockers": blockers,
        "nonClaims": [
...
def main() -> int:
    ...
    try:
        plan = _load_json(args.plan, label="study plan", code="STUDY001")
